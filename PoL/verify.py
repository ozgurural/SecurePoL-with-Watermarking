...
def verify_all(*, model_dir, lr, batch_size, dataset, arch, order, threshold,
               ...
               train_log_level=logging.WARNING, skip_once=True):
    ...
    train_kw_base=dict(lr=lr,batch_size=batch_size,epochs=1,dataset=dataset,augment=augment,
                       architecture=arch,half=0,lambda_wm=0.,k=k,randomize=randomize,
    ...
    for idx,(cur,nxt) in enumerate(zip(ck[:-1],ck[1:])):
        train_kw=train_kw_base.copy(); train_kw["model_dir"]=f"{model_dir}/model_step_{cur}"
        net=_interval_replay(cur,nxt,seq,batch_size,train_level=train_log_level,train_kwargs=train_kw)
        if net is None:
            skipped+=1
        ...
        rows.append({"interval":f"{cur}->{nxt}",**{str(o):v for o,v in zip(order,d)}})
        if writer:
    ...
    if rows:
        ...
        for j,o in enumerate(order):
            viol=int((arr[j]>threshold[j]).sum()); pol_ok&=(viol==0)
...
def verify_topq(*, model_dir, lr, batch_size, dataset, arch, epochs, q, order,
    ...
    base=dict(lr=lr,batch_size=batch_size,epochs=1,dataset=dataset,augment=augment,
              architecture=arch,half=0,lambda_wm=0.,k=k,randomize=randomize,
              watermark_key=watermark_key,watermark_method=watermark_method,
              num_parameters=num_parameters,perturbation_strength=perturbation_strength,
              watermark_size=watermark_size,log_tb=False)
    ...
    for ep in range(epochs):
        ...
        for ind in np.argsort(local)[-q:]:
            ...
            if end<=start:
                logging.warning(f"[skip] {c}->{n} (empty slice)"); continue
            net,*_= _run_train_silent(train_log_level, sequence=seq[start:end],
                                      model_dir=f"{model_dir}/model_step_{c}", **base)
