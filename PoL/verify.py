...
def _extract(m,x,layer="layer1"):
    buf=[]; h=dict(m.named_modules())[layer].register_forward_hook(lambda *_:buf.append(_[2]))
    with torch.no_grad(): m(x); h.remove(); return buf[0]
def _feature_wm_ok(net,dev="cpu")->bool:
    net.to(dev).eval()
    return _extract(net,_dummy_inputs(dev)).mean().item() > 0.01

# ───────── integrity checks ─────────
def _check_hash(d:Path, dataset:str)->bool:
    idx, hfile = d/"indices.npy", d/"hash.txt"
    if not (idx.exists() and hfile.exists()):
        logging.error("[hash] indices.npy or hash.txt missing"); return False
    seq=np.load(idx)
...
def _check_init(d:Path, arch)->bool:
    ck=d/"model_step_0"
    if not ck.exists():
        logging.error("[init‑ks] model_step_0 missing"); return False
    st=torch.load(ck,map_location="cpu")
    wrapped=any(k.startswith("original_model.") for k in st["net"])
    net=WatermarkModule(arch(),"k",128) if wrapped else arch(); net.load_state_dict(st["net"])
...
def _silent_train(lvl, **kw):
    root=logging.getLogger(); prev=root.level; root.setLevel(lvl)
    try: net,*_=train(**kw)
...
p.add_argument("--train-log-level",choices=["ERROR","WARNING","INFO","DEBUG"],
               default="ERROR")
args=p.parse_args()

if len(args.delta)<len(args.dist): args.delta+=args.delta[-1:]*(len(args.dist)-len(args.delta))
if len(args.delta)>len(args.dist): args.delta=args.delta[:len(args.dist)]
...
wm=json.loads(wm_f.read_text()) if wm_f.exists() else {}
...
cfg=dict(
    ...
    train=dict(
        lr=args.lr,batch_size=args.batch_size,epochs=1,dataset=args.dataset,
        augment=args.augment,architecture=arch,half=0,lambda_wm=0.0,
        k=args.k,randomize=args.randomize,
        watermark_key=args.watermark_key,
        watermark_method=args.watermark_method,
        num_parameters=args.num_parameters,
        perturbation_strength=args.perturbation_strength,
        watermark_size=args.watermark_size,log_tb=False),
)
...
with writer as tb:
    if args.q>0:
        pol_ok &= verify_topq(model_dir=out,arch=arch,order=args.dist,q=args.q,
                              epochs=args.epochs,cfg=cfg,writer=tb)
    else:
        pol_ok &= verify_all(model_dir=out,arch=arch,order=args.dist,
                             thr=args.delta,cfg=cfg,writer=tb)
...
if args.watermark_method=="feature_based":
    ...
    if ckpt.exists():
        net=arch(); net.load_state_dict(torch.load(ckpt, map_location=dev)["net"])
        wm_ok=_feature_wm_ok(net,dev)
    else:
        logging.error(f"{ckpt} missing"); wm_ok=False
    ...
    if ckpt.exists():
        ...
        wm_ok=verify_non_intrusive_watermark(net,dev,args.watermark_key,
                                             args.watermark_size,1e-3)
    else: logging.error(f"{ckpt} missing"); wm_ok=False
elif args.watermark_method=="parameter_perturbation":
    ckpt=Path(args.watermark_path)
    if ckpt.exists():
        st=torch.load(ckpt,map_location=dev)
        net=arch(); net.load_state_dict(st["net"])
        wm_ok=verify_parameter_perturbation_watermark_relative(
              net, st.get("original_param_values"), args.watermark_key,
              args.perturbation_strength, 1e-1)
    else: logging.error(f"{ckpt} missing"); wm_ok=False
