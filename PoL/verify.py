...
def verify_hash(model_dir: str, dataset: str) -> None:
    ...
    ok   = hashlib.sha256(data[seq].tobytes()).hexdigest() == \
           Path(model_dir, "hash.txt").read_text().strip()
...
def verify_topq(
    *, model_dir, lr, batch_size, dataset, arch,
    ...
    for ep in range(epochs):
        ...
        local_d = [_dist(f"{model_dir}/model_step_{ck[i]}",
                         f"{model_dir}/model_step_{ck[i+1]}", order, arch)[0]
                   for i in range(st, en-1)]
        for ind in np.argsort(local_d)[-q:]:
            c, n = ck[st+ind], ck[st+ind+1]
            s, e = c*batch_size, min(n*batch_size, len(seq))
            if e <= s:
                logging.debug(f"[skip] {c}->{n} (empty slice)"); continue
...
# ---- read watermark_info.json if present ----
wm_path = Path(out) / "watermark_info.json"
if wm_path.exists():
    wm = json.loads(wm_path.read_text())
    for k,v in wm.items(): setattr(args, k.replace("-", "_"), v)
    seed = wm.get("seed", 777)
else:
    logging.warning(f"{wm_path} not found; assuming --watermark-method=none")
    wm = {"watermark_method": "none"}
    seed = 777
    # fill in missing attributes so later code doesn't crash
    args.watermark_method      = "none"
    args.watermark_key         = ""
    args.k                     = 0
    args.randomize             = False
    args.num_parameters        = 0
    args.perturbation_strength = 0.0
    args.watermark_size        = 0
...
with writer as tb:
    if args.q > 0:
        rec = verify_topq(model_dir=out, lr=args.lr, batch_size=args.batch_size,
                          dataset=args.dataset, arch=arch, epochs=args.epochs,
                          q=args.q, order=args.dist, k=args.k,
        ...
        rec = verify_all(model_dir=out, lr=args.lr, batch_size=args.batch_size,
                         dataset=args.dataset, arch=arch,
                         order=args.dist, threshold=args.delta, k=args.k,
                         randomize=args.randomize, watermark_key=args.watermark_key,
                         watermark_method=args.watermark_method,
                         num_parameters=args.num_parameters,
                         perturbation_strength=args.perturbation_strength,
...
if method == "non_intrusive":
    net = WatermarkModule(arch(), args.watermark_key,
                          watermark_size=wm.get("watermark_size",128))
    net.load_state_dict(torch.load(pth, map_location=dev)["net"])
    ok = verify_non_intrusive_watermark(
            net, dev, args.watermark_key,
            wm.get("watermark_size",128), 1e-3)
    ...
    ok  = verify_parameter_perturbation_watermark_relative(
            net, st.get("original_param_values"),
            args.watermark_key,
