...
import argparse, csv, glob, hashlib, json, logging, os, random
...
def _extract(model, x, layer="layer1"):
    buf=[]; h=dict(model.named_modules())[layer].register_forward_hook(lambda _, __, o: buf.append(o))
    with torch.no_grad(): model(x); h.remove(); return buf[0]

def validate_feature_watermark(model, device="cpu"):
    model.to(device).eval(); score=_extract(model,_dummy_inputs(device)).mean().item()
    ok = score > 0.01
...
def verify_hash(model_dir, dataset):
    ...
    h=hashlib.sha256(); h.update(data[seq].tobytes()); rec=h.hexdigest()
...
def verify_initialization(model_dir, arch, thr=0.01):
    ...
    net.load_state_dict(st["net"]); ks=utils.check_weights_initialization(next(net.parameters()),"resnet")
...
def verify_all(*,model_dir,lr,batch_size,dataset,arch,order,threshold,
    ...
    seq= np.load(os.path.join(model_dir,"indices.npy"))
    order=order if isinstance(order,list) else [order]; threshold=threshold if isinstance(threshold,list) else [threshold]
    rows=[]
    for idx,(cur,nxt) in enumerate(zip(ck[:-1],ck[1:])):
        ...
        row={"interval":f"{cur}->{nxt}",**{str(o):v for o,v in zip(order,d)}}; rows.append(row)
...
def verify_topq(*,model_dir,lr,batch_size,dataset,arch,epochs,q,order,
                ...
                perturbation_strength,watermark_size,writer=None):
    ck=sorted(int(f.split("_")[-1]) for f in glob.glob(f"{model_dir}/model_step_*"))
    seq=np.load(os.path.join(model_dir,"indices.npy")); per_ep=len(ck)//epochs
    order=order if isinstance(order,list) else [order]; rows=[]
    for ep in range(epochs):
        ...
        for ind in np.argsort(local)[-q:]:
            ...
            row={"interval":f"{c}->{n}",str(order[0]):d}; rows.append(row)
...
seed=wm.get("seed",777); torch.manual_seed(seed); torch.cuda.manual_seed_all(seed)
...
pth=args.watermark_path; method=wm.get("watermark_method","none")
if method=="non_intrusive":
    ...
    net=arch(); net.load_state_dict(torch.load(pth,map_location=dev)["net"]); ok=validate_feature_watermark(net,dev)==1.0
