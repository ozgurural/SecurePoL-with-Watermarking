...
def select_parameters_to_perturb(model, num_parameters, watermark_key):
    ...
    Exclude BN parameters to avoid large drift in BN.
    """
    trainable_params = []
    for name, param in model.named_parameters():
        # skip batchnorm stats:
...
def apply_parameter_perturbations(selected_params, watermark_pattern, perturbation_strength):
    """
    Apply small +/- perturbations after optimizer step.
    """
...
def should_embed_watermark(step, k, watermark_key, randomize=False):
    """
    Decide if a watermark should be embedded at 'step'.
    If randomize=True, embed with prob 1/k using a seeded approach.
    """
    if k <= 0:
        return False
    if randomize:
...
def prepare_watermark_data(device='cpu', watermark_key='secret_key'):
    """
    Return a fixed set of inputs (100 samples) for verifying a 'feature-based' watermark.
    """
    num_samples = 100
    input_size = (3, 32, 32)
    seed = int(hashlib.sha256(watermark_key.encode()).hexdigest(), 16) % (2**32)
...
def extract_features(model, inputs, layer_name='layer1'):
    """
    Forward-pass up to `layer_name`. For advanced usage, see code in verify.py or your code.
    """
    features = []
...
def check_watermark_in_features(features, watermark_key, step=0, threshold=0.0001):
    """
    Re-generate the same mask/noise for 'step' and compare with 'features'.
...
def run_feature_based_watermark_verification(model, device='cpu', watermark_key='secret_key'):
    """
    Check for 'feature-based' watermark presence in final model.
    """
...
def verify_non_intrusive_watermark(model, device, watermark_key, watermark_size, tolerance=1e-5):
    """
    Forward pass in trigger=True => produce [batch_size, watermark_size],
    then compare MSE vs. the expected random target.
    """
    model.to(device)
    model.eval()
...
def generate_trigger_inputs(watermark_key, device):
    """
    For Option B: If your model returns [batch_size,10] for CIFAR-10, we pass 10 images.
    """
...
class WatermarkModule(nn.Module):
    """
    Option B: For 'non_intrusive' watermark on CIFAR-10 (model => [batch, 10]).
    If trigger=True, feed that 10D output into an fc => [batch, watermark_size].
    """
    def __init__(self, original_model, watermark_key, watermark_size=128):
        ...
        # Expecting base_out is [batch_size, 10] for CIFAR-10 classification
        self.fc = nn.Linear(10, watermark_size)

    def forward(self, x, trigger=False):
        base_out = self.original_model(x)  # shape [batch_size, 10]
        if trigger:
            return self.fc(base_out)        # [batch_size, watermark_size]
        else:
            return base_out                # normal classification => [batch_size, 10]
...
def verify_parameter_perturbation_watermark_relative(
    ...
    for param-perturbation watermark checks without assuming param=0.
    """
    param_items = []
    for name, param in model.named_parameters():
        if param.requires_grad:
    ...
    if original_params is not None:
        # Only check the originally watermarked
    ...
    for i, (param_name, param_tensor) in enumerate(final_selected):
        ...
        if original_params and param_name in original_params:
            ...
            # fallback if missing => zero-based
